          ...
          <div class="container-fluid">
            <div class="navbar-holder d-flex align-items-center justify-content-between">
              <div class="navbar-header"><a id="toggle-btn" href="#" class="menu-btn"><i class="icon-bars"> </i></a><a href="index.php" class="navbar-brand">
                  <div class="brand-text d-none d-md-inline-block"><strong class="text-primary">Liste des sites</strong></div></a></div>
                    ...
                    <table style="width: 100%;" class="table table-striped">
                        ...
                        <?php
                          ...
                          {?>
                            <tr>
                              <td>
                                <a onclick="window.open(this.href); return false;" style="text-decoration : none; color : #333;" href="<?php echo "http"."://".$donnees['NOM'];?>"><?php echo $donnees['NOM'];?></a>
                              </td>
                              <td><?php echo tronque_chaine($donnees['texte'], 40);?></td>
                              <td><?php echo tronque_chaine($donnees['motcle'], 40);?></td>
                              <?php
                                $auteur= $donnees['auteur'];
                                $sql= "SELECT * FROM profil WHERE ID='$auteur'";
                                $resql = mysqli_query($con, $sql);
                                $data= mysqli_fetch_array($resql); ?>
                              <td><?php echo $data['PRENOMPROFIL'] .' '. $data['NOMPROFIL'];?></td>
                              <td><?php echo date("d/m/Y", strtotime($donnees['datemodif']));?></td>
                              <td> <a href="notes.php?site=<?php echo $donnees['NOM']; ?>" title="Modifier">  <i class="fa fa-pencil" style="font-size:20px;color:blue"></i></a>
                              </td>
                              <?php if(isset($_SESSION['droits']) && $_SESSION['droits']==2){ ?>
                              <td> <a href="scriptBD_deleteSite.php?dn=<?php echo $donnees['id'] ?>" title="Supprimer" onclick="return confirm('Voulez vous supprimer ce site? cette action supprimera aussi les données dans les annuaires');"> <i class="fa fa-trash" style="font-size:20px;color:red"></i></a>
                                </td> 
                                <?php } ?>
                            </tr>
